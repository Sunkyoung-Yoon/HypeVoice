  ...
  return (
    <>
      <Grid item key={props.work.workId} xs={12} sm={6} md={4}>
        <Card style={{ position: 'relative' }} sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
          <div className="cardButton">
            {star == 1 && <StarIcon style={
              {
          ...
          <Modal
            ...
            <Box sx={modalStyle}>
              ...
              <div className="workViewImg" style={{
                width: '200px',
                height: '112.5px',
              ...
              <a href={props.work.videoLink} target="_blank" rel="noreferrer">{props.work.videoLink}</a>
              <button onClick={handleScriptOpen}>{props.work.scriptUrl}</button>
              <button style={{ position: 'absolute', top: 10, right: 10 }} onClick={handleWorkClose}>X</button>
            </Box>
          </Modal>
          ...
          <CardContent style={{paddingBottom: "0px"}} sx={{ flexGrow: 1 }}>
              <Typography gutterBottom variant="h5" component="h2" align="left" onClick={handleWorkOpen}>
                {props.work.title}
              </Typography>
            <Typography align="left">{props.work.info}</Typography>
          </CardContent>
          <div className="existIcon" style={{ display: "flex", justifyContent: "flex-end", marginRight: "5px", marginBottom: "5px"}}>
            <TextSnippetOutlinedIcon fontSize="large" onClick={handleScriptOpen} />
            <Modal
            open={scriptOpen}
            ...
            >
              <Box sx={modalStyle}>
                ...
                <Typography id="modal-modal-description" sx={{ mt: 2 }}>
                  여기에 대본 띄워줘야 합니다. (길어지면 스크롤 넣어야 함)
                </Typography>
                <button style={{ position: 'absolute', top: 10, right: 10 }} onClick={handleScriptClose}>X</button>
              </Box>
            </Modal>

            <OndemandVideoIcon fontSize="large" onClick={handleVideoOpen} />
            <Modal
            open={videoOpen}
            ...
            >
              <Box sx={modalStyle}>
                ...
                <button style={{ position: 'absolute', top: 10, right: 10 }} onClick={handleVideoClose}>X</button>
              </Box>
            </Modal>
          </div>
        </Card>
