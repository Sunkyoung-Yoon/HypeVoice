...
const updateMember = async (accessToken: string, newNickname: string, newProfileUrl: File | null) => {
  ...
  formData.append("request", new Blob([JSON.stringify(memberUpdateRequest)], {type:"application/json"}));

  if (newProfileUrl)
    formData.append("file", newProfileUrl);
...
function MyPage() {
  ...
  useEffect(() => {
    ...
    currentMemberInfo.profileUrl == null ? setProfileUrl("src/assets/HYPE_VOICE_IMG.png") : setProfileUrl(currentMemberInfo.profileUrl);
